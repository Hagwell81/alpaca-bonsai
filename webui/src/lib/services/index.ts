...
/**
 * **DesktopService** - Unified model management client (Electron IPC + HTTP)
 *
 * Bridges model management operations (installed models, HuggingFace search,
 * downloads, deletion) between the Electron desktop app (via window.llamaAPI
 * IPC) and the standalone webui (via HTTP endpoints on the API gateway).
 *
 * @see desktop.service.ts for the transport selection logic
 */
export {
	DesktopService,
	isElectron,
...
/**
 * **VoiceService** - Desktop voice service client for STT and TTS
 *
 * Wraps IPC calls to the desktop VoiceService backend. Provides speech-to-text
 * via whisper.cpp and text-to-speech with browser or MOSS-TTS fallback.
 */
export { VoiceService } from './voice.service';


/**
 * **KnowledgeBaseService** - Knowledge base operations client
 *
 * Wraps window.llamaAPI knowledge base methods for collection management,
 ...
 * Launch Service
 *
 * Wraps window.llamaAPI integration launch methods for configuring
 * external tools (Claude Code, VS Code, Cursor, Discord, etc.) to use
 * the local Alpaca API.
 */
export { LaunchService } from './launch.service';

/**
 * Bonsai Models Service
 *
 * Wraps window.llamaAPI bonsai model catalog methods for listing,
 * checking, and downloading the prerequisite Bonsai ternary, image,
 * TTS, and STT models (mirrors bonsai-beach config/bonsai-beach.toml).
 */
export { BonsaiModelsService } from './bonsai-models.service';

/**
 * Image Service
 *
 * Wraps window.llamaAPI image generation methods for local image generation
 * via sd.cpp (Bonsai Image 4B). Used by the /image route and the inline
 * /imagine chat command.
 */
