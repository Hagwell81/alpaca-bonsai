...
/**
 * Multi-model comparison panel. Displays side-by-side responses from different
 * models for the same user query. Fetches sibling assistant messages from the
 * conversation tree and renders them in a grid layout when multi-model mode
 * is enabled in settings.
 */
export { default as ChatComparePanel } from './ChatMessages/ChatComparePanel.svelte';
...
/**
 * Integrations tab content for the settings panel. Lists all available
 * integrations with install status indicators and Launch buttons. When
 * Launch is pressed, checks if the tool is installed and either launches
 * it with Alpaca configuration or shows an "not installed" message.
 */
export { default as ChatSettingsIntegrationsTab } from './ChatSettings/ChatSettingsIntegrationsTab.svelte';
...
/**
 * **KnowledgeBasePanel** - Main knowledge base UI panel
 *
 * Collection list sidebar, document list, search interface with results,
 * and ingest buttons for files and URLs.
 */
export { default as KnowledgeBasePanel } from '../knowledgebase/KnowledgeBasePanel.svelte';

/**
 * **WorkspacePanel** - Workspace UI panel
 *
 * Shows current folder path, buttons to select local folder or open sandbox,
 * and a simplified file tree display.
 */
export { default as WorkspacePanel } from '../workspace/WorkspacePanel.svelte';

/**
 * **IntegrationPanel** - External tool integrations panel
 *
 * One-click configuration for IDEs, coding agents, chat bots,
 * automation tools, and notebooks to use the local API.
 */
export { default as IntegrationPanel } from '../integrations/IntegrationPanel.svelte';
