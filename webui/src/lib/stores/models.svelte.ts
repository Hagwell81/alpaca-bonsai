...
class ModelsStore {
	...
	/**
	 * Async check if a model supports vision, with IPC fallback.
	 * Use this when you need an authoritative answer and can await.
	 */
	async modelSupportsVisionAsync(modelId: string): Promise<boolean> {
		const modalities = this.getModelModalities(modelId);
		if (modalities) return modalities.vision ?? false;

		// Last-resort fallback: check desktop IPC for the active model's vision capability
		try {
			const api = (window as any).llamaAPI;
			if (api?.getActiveModel) {
				const activeModel = await api.getActiveModel();
				if (activeModel?.hasVision) return true;
			}
	...
	async fetch(force = false): Promise<void> {
		...
		try {
			...
			// FALLBACK: If /props didn't include modalities (older llama-server
			// versions or the legacy single-server path), check the desktop IPC
			// get-active-model handler which knows about mmproj/vision pairing.
			// This ensures the Bonsai 27B model is correctly detected as vision-capable
			// even when llama-server's /props response lacks the modalities field.
			if (serverStore.isModelMode && this.models.length > 0) {
				const hasModalitiesFromProps = this.models.some(m => m.modalities?.vision);
				if (!hasModalitiesFromProps) {
					try {
						const api = (window as any).llamaAPI;
						if (api?.getActiveModel) {
							const activeModel = await api.getActiveModel();
							if (activeModel?.hasVision) {
								const modalities: ModelModalities = {
									vision: true,
									audio: false
								};
								...
								// Also cache the props with modalities so modelSupportsVision() works
								if (serverProps) {
									this.modelPropsCache.set(this.models[0].model, {
										...serverProps,
										modalities
									});
								}
						...
						// IPC not available (e.g. running outside Electron) — silently ignore
					}
				}
			}
		} catch (error) {
