...
class ChatStore {
	...
	// Multi-model support: background streams for compare models
	private backgroundLoadingStates = new SvelteMap<string, boolean>();
	private backgroundStreamingStates = new SvelteMap<
		string,
	...
	private getOrCreateAbortController(convId: string, suffix?: string): AbortController {
		const key = suffix ? `${convId}:${suffix}` : convId;
		let c = this.abortControllers.get(key);
	...
	private abortRequest(convId?: string, suffix?: string): void {
		if (convId && suffix) {
			const key = `${convId}:${suffix}`;
			...
			// Abort all controllers matching this convId (with and without suffix)
			for (const [key, c] of this.abortControllers.entries()) {
				if (key === convId || key.startsWith(`${convId}:`)) {
					c.abort();
					this.abortControllers.delete(key);
				}
			}
	...
	/**
	 * Get the list of compare model IDs from settings.
	 * Returns empty array if multi-model is disabled or not in router mode.
	 */
	getCompareModels(): string[] {
		const currentConfig = config();
		if (!currentConfig.multiModelEnabled || !isRouterMode()) return [];
		const ids = currentConfig.multiModelIds;
	...
	/**
	 * Check if multi-model comparison is active for this conversation.
	 */
	isMultiModelActive(convId: string): boolean {
		return this.backgroundLoadingStates.has(convId) || this.getCompareModels().length > 0;
	}

	/**
	 * Get background streaming state for a specific branch.
	 */
	getBackgroundStreamingState(stateKey: string): { response: string; messageId: string } | undefined {
		return this.backgroundStreamingStates.get(stateKey);
	}
	...
	/**
	 * Handle the inline `/imagine <prompt>` chat command.
	 *
	 * Generates an image locally via sd.cpp (Bonsai Image 4B) through the
	 * desktop ImageService, then inserts the user's command as a normal
	 * user message and an assistant message containing the generated image
	 * rendered as a markdown image with a data URI.
	 */
	async handleImagineCommand(prompt: string, rawCommand: string): Promise<void> {
		const activeConv = conversationsStore.activeConversation;
		...
		try {
			// 1. Add the user message (the /imagine command text)
			...
			// 3. Call the ImageService
			const result = await ImageService.generate({ prompt, b64: true });
			const assistantIdx = conversationsStore.findMessageIndex(assistantMessage.id);
			if (!result.success || !result.b64) {
				const errMsg = result.error || 'Image generation failed';
				const errorContent = `⚠️ Image generation failed: ${errMsg}`;
				await DatabaseService.updateMessage(assistantMessage.id, { content: errorContent });
				if (assistantIdx !== -1)
					conversationsStore.updateMessageAtIndex(assistantIdx, { content: errorContent });
				this.showErrorDialog({ type: ErrorDialogType.SERVER, message: errMsg });
				return;
			}

			// 4. Update the assistant message with the generated image
			const markdown = `![generated image](data:image/png;base64,${result.b64})\n\n*Prompt:* ${prompt}`;
			await DatabaseService.updateMessage(assistantMessage.id, { content: markdown });
			if (assistantIdx !== -1)
				conversationsStore.updateMessageAtIndex(assistantIdx, { content: markdown });
		} catch (error) {
			console.error('Failed to handle /imagine command:', error);
	...
	async sendMessage(content: string, extras?: DatabaseMessageExtra[]): Promise<void> {
		...
		// Inline /imagine command: generate an image locally via sd.cpp
		// (Bonsai Image 4B) and insert it as an assistant message containing
		// a markdown image. The command is not forwarded to the LLM.
		const imagineMatch = content.match(/^\/imagine(?:\s+(.*))?$/s);
		if (imagineMatch) {
			const prompt = (imagineMatch[1] || '').trim();
			if (!prompt) {
				this.showErrorDialog({ type: ErrorDialogType.SERVER, message: 'Usage: /imagine <prompt>' });
				return;
			}
			await this.handleImagineCommand(prompt, content);
			return;
		}
		...
		try {
			...
			const primaryModel = isRouterMode() ? selectedModelName() : null;
			const effectiveCompareModels = compareModels.filter(
				(m) => m !== primaryModel && modelsStore.isModelLoaded(m)
			);
			...
			const primaryStream = this.streamChatCompletion(
				conversationsStore.activeMessages.slice(0, -1),
				assistantMessage
			);

			// Start background streams for compare models concurrently
			const backgroundStreams: Promise<void>[] = [];
			if (effectiveCompareModels.length > 0) {
				for (const modelName of effectiveCompareModels) {
					const branchMsg = await DatabaseService.createMessageBranch(
						{
					...
					this.backgroundLoadingStates.set(`${currentConv.id}:${branchMsg.id}`, true);
					backgroundStreams.push(
						this.streamChatCompletionBranch(
							conversationsStore.activeMessages.slice(0, -1),
							branchMsg,
							modelName,
							currentConv.id
			...
			if (backgroundStreams.length > 0) {
				await Promise.allSettled(backgroundStreams);
			}
		} catch (error) {
	...
	/**
	 * Stream a chat completion for a background branch (multi-model comparison).
	 * Does not update the main conversation loading state.
	 */
	private async streamChatCompletionBranch(
		allMessages: DatabaseMessage[],
		assistantMessage: DatabaseMessage,
		modelOverride: string,
		convId: string
	): Promise<void> {
		if (!modelsStore.isModelLoaded(modelOverride)) return;

		let streamedContent = '';
		let streamedReasoningContent = '';
		const currentMessageId = assistantMessage.id;
		const stateKey = `${convId}:${currentMessageId}`;

		const abortController = this.getOrCreateAbortController(convId, currentMessageId);
		...
		try {
			await ChatService.sendMessage(
				allMessages,
				{
					...
					onChunk: (chunk: string) => {
						streamedContent += chunk;
						this.backgroundStreamingStates.set(stateKey, {
					...
					onModel: (modelName: string) => {
						const n = normalizeModelName(modelName);
						if (n) {
							DatabaseService.updateMessage(currentMessageId, { model: n }).catch(console.error);
						}
					},
					onComplete: async (
						finalContent?: string,
						...
						await DatabaseService.updateMessage(currentMessageId, {
							content,
							reasoningContent: streamedReasoningContent || reasoningContent || undefined,
							timings
						});
						this.backgroundLoadingStates.delete(stateKey);
						this.backgroundStreamingStates.delete(stateKey);
						if (isRouterMode()) modelsStore.fetchRouterModels().catch(console.error);
					},
					onError: (error: Error) => {
						if (isAbortError(error)) {
						...
						DatabaseService.updateMessage(currentMessageId, {
							content: streamedContent || 'Error: ' + error.message
						}).catch(console.error);
					}
				},
				convId,
	...
	private getApiOptions(): Record<string, unknown> {
		...
		if (currentConfig.reasoningFormat) apiOptions.reasoningFormat = currentConfig.reasoningFormat;
		...
		if (hasValue(currentConfig.seed)) apiOptions.seed = Number(currentConfig.seed);
		if (hasValue(currentConfig.mirostat)) apiOptions.mirostat = Number(currentConfig.mirostat);
		if (hasValue(currentConfig.mirostat_tau))
			apiOptions.mirostat_tau = Number(currentConfig.mirostat_tau);
		if (hasValue(currentConfig.mirostat_eta))
			apiOptions.mirostat_eta = Number(currentConfig.mirostat_eta);
		if (hasValue(currentConfig.n_keep)) apiOptions.n_keep = Number(currentConfig.n_keep);
		if (hasValue(currentConfig.n_discard)) apiOptions.n_discard = Number(currentConfig.n_discard);
		if (currentConfig.ignore_eos) apiOptions.ignore_eos = true;
		if (currentConfig.grammar) apiOptions.grammar = currentConfig.grammar;
		if (currentConfig.grammar_lazy) apiOptions.grammar_lazy = true;
		if (currentConfig.stop) apiOptions.stop = currentConfig.stop;
		if (currentConfig.logit_bias) apiOptions.logit_bias = currentConfig.logit_bias;
		if (hasValue(currentConfig.n_probs)) apiOptions.n_probs = Number(currentConfig.n_probs);
		if (hasValue(currentConfig.min_keep)) apiOptions.min_keep = Number(currentConfig.min_keep);
		if (hasValue(currentConfig.top_n_sigma))
			apiOptions.top_n_sigma = Number(currentConfig.top_n_sigma);
		if (currentConfig.post_sampling_probs) apiOptions.post_sampling_probs = true;
		if (currentConfig.chat_format) apiOptions.chat_format = currentConfig.chat_format;
